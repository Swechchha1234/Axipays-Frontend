...
const MidModal = ({
    ...
    const handleLoginClick = () => {
        navigate("/auth?form=login");  // Navigate to login page
    ...
    const handleOtpSubmit = () => {
        if (otp.every((digit) => digit !== "")) {
            handleOtpVerification(otp.join("")); // Join the array to get the full OTP
            setStep(3);
        }
    };
    ...
    const handleOtpChange = (value, index) => {
        ...
        updatedOtp[index] = value.slice(-1); // Only allow one digit per box
        setOtp(updatedOtp);
