...
const TransactionMonitoring = () => {
	...
	const fetchDistinctValuesEndpoint =
		"api/v1/utility/fetch_distinct_values?column=";
	...
	const [headerLabels] = useState([
		...
		{ heading: "Merchant Txn ID", label: "client_txn_id" },
	...
	const handleSearch = async (page, rows) => {
		...
		// console.log("dsfdsgffd",searchedData)
		try {
	...
	const fetchData = async (type) => {
		...
		try {
			...
			if (type === "company") {
				endpoint = "api/v1/client/?detailed=true";
				mapper = (item) => item.client_ref_id;
			...
			if (response.data) {
				...
				if (type === "company") {
					setMerchantOptions(mappedData);
				} else if (type === "acquirer") {
					setPaymentgatewayOptions(mappedData);
				} else if (type === "mid") {
					setMidOptions(mappedData);
				}
			}
		} catch (error) {
				...
				<div className="main-screen">
						...
						<div>
							<div className="id-search-row-div">
								...
								<div className="search-clear-btn">
									...
									<Button onClick={() => handleSearch(currentPage, rowsPerPage)} className="btn-medium" backgroundcolor="#003366" size="medium">
										{loading ? (
					...
					<div className={`more-show ${showMoreOptions ? "show" : "hide"}`} style={{
						...
						<div className="show-more-div">
							...
							/>
							<Select
								label={role === "client" ? "Currency" : "MID"}
								id={role === "client" ? "currency" : "mid"}
								options={role === "client" ? currencyOptions : midOptions}
								value={role === "client" ? filters.currency : filters.mid}
								onChange={role === "client" ? (value) => handleInputChange('currency', value) : (value) => handleInputChange('mid', value)}
								onKeyDown={handleKeyDown}
								className="showMore-input"
							/>
							<Select
					...
					<div className="transaction-monitoring">
						<div>
							<div className="id-search-row-div">
								...
								<Button onClick={handleSearch} className="btn-medium" backgroundcolor="#003366" size="medium">
									{loading ? (
										<Loader strokeColor="#fafafa" width={20} height={20} />
									) : (
										<>
